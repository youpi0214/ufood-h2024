...
export const extractUniqueAttributeListFromExistingList = (
  unfilteredList,
  filterAttribute,
) => {
  return unfilteredList.reduce((accumulator, currator) => {
    const existing = accumulator.find(
      (visitInAccumulator) =>
        visitInAccumulator[filterAttribute] === currator[filterAttribute],
    );
    if (!existing) {
...
export const getAllAvailableDataWithQueryFunction = async (
  queryFunction,
  functionParameters,
  queryQuantity,
) => {
  ...
  for (let i = 0; i < totalQueries; i++) {
    ...
    const [instancelist, _] = await queryFunction(
      ...functionParameters,
      options,
    );
